...
export default function Reports({ agents = [] }) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 p-4 sm:p-6 mb-6 sm:mb-8">
        <h1 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white mb-4 sm:mb-6">Historical Performance Report</h1>
        
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-4 items-end">
          <div>
            <label className="block text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider mb-1">Start Date</label>
            <input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} className="w-full bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-600 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 text-gray-900 dark:text-white" />
          </div>
          <div>
            <label className="block text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider mb-1">End Date</label>
            <input type="date" value={endDate} onChange={e => setEndDate(e.target.value)} className="w-full bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-600 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 text-gray-900 dark:text-white" />
          </div>
          <div>
            <label className="block text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider mb-1">Team</label>
            <select value={teamFilter} onChange={e => setTeamFilter(e.target.value)} className="w-full bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-600 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 text-gray-900 dark:text-white">
              <option value="all">All Teams</option>
            </select>
          </div>
          <div>
            <label className="block text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider mb-1">Sort By</label>
            <div className="flex gap-2 w-full">
              <select value={sortBy} onChange={e => setSortBy(e.target.value)} className="flex-1 min-w-0 bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-600 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 text-gray-900 dark:text-white">
              ...
              <button 
                ...
                className="shrink-0 bg-gray-100 dark:bg-slate-800 border border-gray-200 dark:border-slate-600 rounded-lg px-3 py-2 text-sm font-bold text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-slate-700 transition-colors"
          ...
          <div>
            <button onClick={fetchReports} disabled={loading} className="w-full bg-orange-500 hover:bg-orange-600 text-white font-bold py-2 px-6 rounded-lg text-sm transition-colors cursor-pointer disabled:opacity-50 shadow-sm">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 overflow-hidden">
        {/* Mobile Swipe Hint */}
        <div className="sm:hidden px-3.5 py-2 bg-orange-50/70 dark:bg-slate-900/60 text-[11px] font-medium text-orange-700 dark:text-orange-300 flex items-center justify-between border-b border-orange-100 dark:border-slate-700/60">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs sm:text-sm text-left whitespace-nowrap">
            <thead className="text-[11px] sm:text-xs text-gray-500 uppercase bg-gray-50 dark:bg-slate-700/50 dark:text-gray-400">
              <tr>
                <th className="px-3.5 sm:px-6 py-3 sticky left-0 z-10 bg-gray-50 dark:bg-slate-800 shadow-[1px_0_0_0_rgba(0,0,0,0.05)] dark:shadow-[1px_0_0_0_rgba(255,255,255,0.05)]">Name</th>
                <th className="px-3 sm:px-6 py-3">Tenure (Days)</th>
                <th className="px-3 sm:px-6 py-3">Talk Time</th>
                <th className="px-3 sm:px-6 py-3">Total Dials</th>
                <th className="px-3 sm:px-6 py-3">Unique Calls</th>
                <th className="px-3 sm:px-6 py-3">Connected Calls</th>
                <th className="px-3 sm:px-6 py-3">Long Calls (5m+)</th>
              </tr>
            </thead>
            <tbody>
              {/* Summary Row */}
              <tr className="bg-orange-50/50 dark:bg-orange-900/10 font-bold border-b-2 border-gray-200 dark:border-slate-600">
                <td className="px-3.5 sm:px-6 py-3.5 text-orange-800 dark:text-orange-300 sticky left-0 z-10 bg-orange-50/90 dark:bg-slate-800 shadow-[1px_0_0_0_rgba(0,0,0,0.05)] dark:shadow-[1px_0_0_0_rgba(255,255,255,0.05)]">TOTAL</td>
                <td className="px-3 sm:px-6 py-3.5 text-orange-800 dark:text-orange-300">-</td>
                <td className="px-3 sm:px-6 py-3.5 text-orange-800 dark:text-orange-300">
              ...
              {[...activeReports].sort((a, b) => {
                ...
                <tr key={report.agentId} className="border-b dark:border-slate-700/50 hover:bg-gray-50 dark:hover:bg-slate-800/50">
                  <td className="px-3.5 sm:px-6 py-3 sm:py-4 font-medium text-gray-900 dark:text-white sticky left-0 z-10 bg-white dark:bg-slate-800 shadow-[1px_0_0_0_rgba(0,0,0,0.05)] dark:shadow-[1px_0_0_0_rgba(255,255,255,0.05)]">
                  ...
                  <td className="px-3 sm:px-6 py-3 sm:py-4 text-gray-600 dark:text-gray-400">{report.tenure}</td>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-4xl overflow-hidden flex flex-col max-h-[90vh] sm:max-h-[85vh] border border-gray-200 dark:border-slate-700">
            {/* Modal Header */}
            <div className="p-3.5 sm:p-4 sm:px-6 bg-gradient-to-r from-orange-500 to-amber-600 dark:from-slate-900 dark:to-slate-800 border-b border-orange-600 dark:border-slate-700 flex justify-between items-center text-white gap-3">
              <div className="min-w-0 flex-1">
                <h3 className="text-base sm:text-lg font-bold flex items-center gap-2 truncate">
                  <span>📞</span> <span className="truncate">{modalTitle}</span>
                </h3>
                <p className="text-[11px] sm:text-xs text-orange-100 dark:text-slate-400 mt-0.5 truncate">
                  Agent: <strong className="text-white font-semibold">{selectedAgent?.name}</strong> • Range: {startDate} to {endDate}
                </p>
              </div>
              <button
                onClick={() => setShowModal(false)}
                className="w-8 h-8 rounded-full bg-white/20 hover:bg-white/30 text-white flex items-center justify-center transition-colors cursor-pointer font-bold text-lg shrink-0"
                title="Close modal"
              >
                &times;
              </button>
              ...
              {loadingLongCalls ? (
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-xs sm:text-sm text-left border-collapse whitespace-nowrap">
                    <thead className="text-[10px] sm:text-xs text-gray-500 dark:text-slate-400 uppercase bg-gray-50 dark:bg-slate-900/60 border-b border-gray-200 dark:border-slate-700">
                      <tr>
                        <th className="px-3 sm:px-4 py-2.5 sm:py-3">Lead / Contact Name</th>
                        <th className="px-3 sm:px-4 py-2.5 sm:py-3">Contact Number</th>
                        <th className="px-3 sm:px-4 py-2.5 sm:py-3">Duration</th>
                        <th className="px-3 sm:px-4 py-2.5 sm:py-3">Timestamp</th>
                        <th className="px-3 sm:px-4 py-2.5 sm:py-3 text-right">Status</th>
                      </tr>
                    </thead>
