...
export default function GlobalSettings({ products = [], setProducts, statuses = [], setStatuses, API_URL, token }) {
  ...
  const handleAddStatus = (e) => {
    ...
    if (localStatuses.some(s => s.toLowerCase() === trimmed.toLowerCase())) {
      toast.error('This status option already exists.');
      return;
    }

    setLocalStatuses([...localStatuses, trimmed]);
    setNewStatus('');
  };
  ...
  const handleRemoveStatus = (indexToRemove) => {
    ...
    setLocalStatuses(localStatuses.filter((_, index) => index !== indexToRemove));
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (prodRes.ok && statusRes.ok) {
        const prodData = await prodRes.json();
        const statusData = await statusRes.json();
        if (setProducts) setProducts(prodData.products || []);
        if (setStatuses) setStatuses(statusData.statuses || []);
        toast.success('Global configuration updated successfully.');
      } else {
        toast.error('Failed to update global config changes.');
      }
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-slate-400">
            Manage global configuration and dropdown options for the CRM.
          </p>
        </div>
        <div>
          <button
            onClick={handleSave}
            disabled={isSaving}
            className="inline-flex items-center justify-center py-2.5 px-6 border border-transparent shadow-md text-sm font-bold rounded-xl text-white bg-orange-600 hover:bg-orange-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500 disabled:opacity-50 transition-all cursor-pointer"
          >
            {isSaving ? 'Saving Changes...' : 'Save All Config Changes'}
          </button>
        </div>
      </div>
        ...
        <div className="p-6 sm:p-8">
          <div className="mb-8 border-b border-gray-200 dark:border-slate-700/80 pb-6">
            ...
            <p className="text-sm text-gray-500 dark:text-slate-400">
              Manage the lead status options available across Dashboard, Reports, Add Lead, and Lead Detail views.
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="flex flex-col h-full">
              <h4 className="text-sm font-semibold text-gray-700 dark:text-slate-300 uppercase tracking-wider mb-4">Current Statuses</h4>
              <div className="space-y-3 flex-grow overflow-y-auto max-h-[300px] pr-2">
                {localStatuses.map((status, index) => (
                  <div key={index} className="flex justify-between items-center bg-gray-50/80 dark:bg-slate-900/50 px-4 py-3 rounded-xl border border-gray-200 dark:border-slate-700/80 hover:bg-white dark:hover:bg-slate-800 transition-colors group shadow-sm">
                    <span className="text-sm font-bold text-gray-800 dark:text-slate-200">{status}</span>
                    <button
                      onClick={() => handleRemoveStatus(index)}
                      className="text-gray-400 hover:text-red-600 dark:text-slate-500 dark:hover:text-red-400 transition-colors p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 cursor-pointer"
                      title="Remove status"
                    >
            ...
            <div className="flex flex-col border-t md:border-t-0 md:border-l border-gray-200 dark:border-slate-700/80 pt-6 md:pt-0 md:pl-8">
              <h4 className="text-sm font-semibold text-gray-700 dark:text-slate-300 uppercase tracking-wider mb-4">Add New Status Option</h4>
              <form onSubmit={handleAddStatus} className="flex flex-col gap-3">
                <input
                  type="text"
                  value={newStatus}
      ...
      {/* Section 2: Products Dropdown Options */}
      <div className="bg-white dark:bg-slate-800/80 backdrop-blur shadow-sm rounded-2xl overflow-hidden border border-gray-200 dark:border-slate-700/80 transition-colors">
        <div className="p-6 sm:p-8">
          <div className="mb-8 border-b border-gray-200 dark:border-slate-700/80 pb-6">
            <h3 className="text-xl font-bold text-gray-800 dark:text-gray-100 flex items-center gap-2 mb-2">
